...
public class Main {
 ...
 int goal ;
 int limit;
 Main(int size,int loc, int goald,int inlimit)
 {
 N=size;
 currPos=loc;
 goal=goald;
...
sta.push(i);
 ...
 if (limit<level && !found) {
 System.out.println("Cutoff");
 return;
 if (level == limit && !found) {
 ...
 Main traGraph=new Main(12,cityChoice,goalcity,inplimit);
 traGraph.DLS();
 }
